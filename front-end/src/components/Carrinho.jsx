...
const Carrinho = () => {
  ...
  const fecharPedido = async () => {
    const { data } = await axios.post('http://localhost:3001/pedido/', {
      valorTotal,
  ...
  return (
    <div className="carrinho">
      <div className="carrinho__titulo">
        <h2>Carrinho</h2>
        <p onClick={() => setCarrinhoAberto(false)}>X</p>
      </div>
      {itensCarrinho.length > 0 ? (
        <>
          <div className="carrinho__produtos">
            {itensCarrinho.map(({ quantidade, produto }, index) => (
              <div key={index} className="carrinho__produto">
                ...
                <div className="carrinho__quantidade">
                  <div className="carrinho__icone-qtd">
                    <p
                      className="carrinho__sinal-menos"
                    ...
                    <p>{quantidade}</p>
                    <p
                      className="carrinho__sinal-mais"
          ...
          <div className="carrinho__pedido">
            ...
            <button onClick={() => fecharPedido()}>Finalizar pedido</button>
          </div>{' '}
        </>
      ) : (
        <p className="carrinho__vazio">Seu carrinho esta vazio!</p>
      )}
    </div>
  );
};
